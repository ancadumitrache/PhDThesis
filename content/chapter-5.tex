\chapter{Ambiguity in Crowdsourcing Frame Disambiguation}
\label{chap:frames}
...
Against that positivism [\ldots] saying ``there are only facts,'' I should say: no, it is precisely facts that do not exist, only interpretations.
\end{chapquote}
...
This chapter was published as ``Capturing Ambiguity in Crowdsourcing Frame Disambiguation'' in AAAI Conference on Human Computation and Crowdsourcing~(\citet{DBLP:conf/hcomp/DumitracheAW18}) and was co-authored by Lora Aroyo and Chris Welty.
...
There have been a number of attempts at using crowdsourcing for frame disambiguation in sentences, such as those by \citet{Hong:2011:GCR:2018966.2018970} and \citet{chang2015scaling}, offering a creative way to deal with the complexity of the annotation task. This paper addresses the considerable problem of \emph{ambiguity}  in frame annotation, which we show to be a prominent feature in frame semantics.  We adapt the CrowdTruth framework, which encourages using multiple crowd annotators to perform the same work, and processes the disagreement between them to signal low quality workers, sentences, and frames.
...
For simplicity, we refer to the sentence-word pairs as sentences in the rest of the paper. This dataset, as well as the crowdsourcing results and aggregated metrics are available online~\cite{anca_dumitrache_2018_1472345}.
...
\centering
\caption{F1 score of crowd using expert \\ annotation as true positives}
\label{fig:sent_f1}
\includegraphics[width=\linewidth]{img/sent_f1_micro_macro.pdf}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
\centering
\caption{Accuracy of crowd annotations using expert annotation as correct}
\label{fig:sent_acc}
...
%\hline
\textbf{\#} & \textbf{Sentence} & \textbf{Frame} & \textbf{FSS} \\ \hline \hline
...
\multirow{2}{*}{S2} & \multirow{2}{8cm}{The major \textit{changes} were not to daily tasks and routines , but to the political power base.} & $cause\_change$ & 0.804 \\  \cline{3-4}
& & $undergo\_change^{(*)}$ & 0.305 \\ \hline

\multirow{2}{*}{S3} & \multirow{2}{8cm}{This \textit{investigation} has been stymied stopped, obstructions thrown up every step of the way.} & $criminal\_investigation$ & 0.898 \\  \cline{3-4}
& & $scrutiny^{(*)}$ & 0.377 \\ \hline

\multirow{2}{*}{S4} & \multirow{2}{8cm}{Does supersizing \textit{cause} obesity?} & $cause\_to\_start$ & 0.804 \\  \cline{3-4}
& & $causation^{(*)}$ & 0.608 \\ \hline

\multirow{4}{*}{S5} & \multirow{4}{8cm}{The loud, raucous Jamaican English dialect and the \textit{waving} hands reflect the joy with which social relations are conducted here.} & \multirow{2}{*}{$body\_movement$} & \multirow{2}{*}{0.861} \\
& & & \\ \cline{3-4}
...
\begin{table}[tb!]
\centering
\caption{Different FSS values for the frames $removing$ (P1, P2, P3), $means$ (P4, P5, P6), $attempt\_suasion$ (P7, P8, P9). The targeted word appears in italics font in the sentence. The frame picked by the expert is marked with $^{(*)}$.}
\label{tab:fss_ex}

\scalebox{0.8}{
\begin{tabular}{cp{5.5cm}ccc}
%\hline
\textbf{\#} & \textbf{Sentence} & \textbf{SQS} &  \textbf{Frame} & \textbf{FSS}  \\ \hline \hline
...
& & & $cause\_change$ & 0.175 \\ \cline{4-5}
& & & $event$ & 0.032 \\
& & & &  \\ \hline

\multirow{4}{*}{P2} & \multirow{4}{5.5cm}{First, he forbade seeking the aid of infidels when the Syrian Mujahiddin asked Saddam Hussein to \textit{overthrow} the regime of Hafiz Al-Assad in Syria.} & \multirow{4}{*}{0.669} & $change\_of\_leadership^{(*)}$ & 0.847 \\  \cline{4-5}
& & & $removing$ & 0.539 \\ \cline{4-5}
& & & $eventive\_cognizer\_affecting$ & 0.087 \\ \cline{4-5}
& & & $people$ & 0.005 \\ \hline
...
& & & $committing\_crime$ & 0.459 \\ \cline{4-5}
& & & $misdeed$ & 0.431 \\ \cline{4-5}
& & & $cause\_change$ & 0.273 \\ \hline \hline
...
& & & &  \\ \cline{4-5}
& & & \multirow{2}{*}{$being\_employed$} & \multirow{2}{*}{0.11} \\
& & & & \\ \hline
...
& & & $path\_traveled$ & 0.159 \\ \cline{4-5}
& & & $communication$ & 0.121 \\ \hline \hline
...
& & & $attempt\_suasion^{(*)}$ & 0.81 \\ \cline{4-5}
& & & $request$ & 0.387 \\ \cline{4-5}
& & & $communication$ & 0.337 \\ \cline{4-5}
& & & $cause\_to\_start$ & 0.115 \\
& & & & \\ \hline

\multirow{4}{*}{P8} & \multirow{4}{5.5cm}{The security team should \textit{urge} everyone to take precautions and guard their homes tightly.} & \multirow{4}{*}{0.358} & $attempt\_suasion^{(*)}$ & 0.605 \\ \cline{4-5}
& & & $request$ & 0.321 \\ \cline{4-5}
& & & $cause\_to\_start$ & 0.256 \\ \cline{4-5}
& & & $communication$ & 0.213 \\ \hline

\multirow{4}{*}{P9} & \multirow{4}{5.5cm}{The security team should publish a periodic bulletin and distribute to all residents, \textit{advising} them how to safely store gaz and logs.} & \multirow{4}{*}{0.386} & $attempt\_suasion^{(*)}$ & 0.576 \\ \cline{4-5}
...
& & & & \\ \cline{4-5}
%& & & $cause\_to\_start$ & 0.156 \\
%& & & $coming\_up\_with$ & 0.069 \\
& & & \multirow{2}{*}{$create\_physical\_artwork$} & \multirow{2}{*}{0.056} \\
& & & & \\ \hline
...
& & & & \\  \cline{4-5}
& & & \multirow{2}{*}{$create\_physical\_artwork$} & \multirow{2}{*}{0.089} \\
& & & & \\ \hline
% & & & $cause\_change$ & 0.082 \\ 
% & & & $cause\_to\_start$ & 0.082 \\ \hline
...
& & & $create\_physical\_artwork$ & 0.335 \\ \cline{4-5}
& & & $manufacturing$ & 0.237 \\
& & & & \\ \hline \hline
%& & & $cause\_to\_start$ & 0.072 \\ 
%& & & $opinion$ & 0.004 \\ \hline \hline
...
& & & $cause\_change$ & 0.24 \\ \cline{4-5}
& & & $undergo\_change$ & 0.212 \\
& & & & \\ \hline
...
& & & $undergo\_change$ & 0.783 \\  \cline{4-5}
& & & $cause\_change$ & 0.402 \\
& & & & \\ \hline \hline
...
\item $attempt\_suasion$: This frame refers to a speaker attempting to influence the addressee to act. Sentences $P7$ to $P9$ express various degrees of persuasion, from obviously to weakly expressed. In $P7$, it is clear that the attempt at persuasion is an event that has occurred (\textit{Sharon [...] urged}). $P8$ expresses an obligation at an attempt to persuade (\textit{should urge}), whereas in $P9$ the persuasion is weaker, merely \textit{advice}.
...
%\hline
\textbf{Frame} & \textbf{FQS} & \textbf{Definition} & \textbf{Example Sentences} & \textbf{FSS} \\ \hline \hline
