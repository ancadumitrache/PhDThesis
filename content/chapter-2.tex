...
This chapter was published as ``Crowdsourcing Ground Truth for Medical Relation Extraction'' in the ACM Transactions on Interactive Intelligent Systems~(\citet{DBLP:journals/corr/DumitracheAW17}) and was co-authored by Lora Aroyo and Chris Welty.
...
%\hline
\multirow{2}{*}{\textbf{Relation}} & \textbf{Corresponding} & \multirow{2}{*}{\textbf{Definition}} & \multirow{2}{*}{\textbf{Example}} \\ 
 & \textbf{UMLS Relation(s)} &  &   \\ \hline \hline

\textit{treat} & may treat &  therapeutic use of 
a drug &  penicillin treats infection \\ \hline

\textit{cause} & \parbox[t]{3.5cm}{cause of; \\ has causative agent} & the underlying reason for a symptom or a disease &   fever induces dizziness \\  \hline

\textit{prevent} & may prevent &  preventative use of
a drug &  vitamin C prevents influenza \\  \hline

\textit{diagnoses} & may diagnose & diagnostic use of an ingredient, test or a drug &  RINNE test is used to diagnose hearing loss \\  \hline
...
\textit{symptom} & disease has finding; disease may have finding & deviation from normal function indicating the presence of disease or abnormality &  pain is a symptom of a broken arm \\  \hline
...
\textit{contraindicate} & contraindicated drug & a condition for which a drug or treatment should not be used &  patients with obesity should avoid using danazol \\  \hline
...
\textit{associated with} & associated with & signs, symptoms or findings that often appear together &  patients who smoke often have yellow teeth \\  \hline
...
\textit{part of} & part of & an anatomical or structural sub-component &  the left ventricle is part of the heart \\  %\hline
\end{tabular}
%\egroup
}
\end{center}
...
\multirow{3}{*}{\textbf{Relation}} & \multicolumn{2}{c}{\textbf{sentence}} &
\multicolumn{2}{c}{\textbf{sentence-relation}} & \multicolumn{2}{c}{\textbf{crowd score used in}} \\
& \multicolumn{2}{c}{\textbf{vector}} & \multicolumn{2}{c}{\textbf{score}} & \multicolumn{2}{c}{\textbf{model training}} \\ \cline{2-7}
& \textit{Sent.1} & \textit{Sent.2} & \textit{Sent.1} & \textit{Sent.2} & \textit{Sent.1} & \textit{Sent.2} \\ \hline
\textit{treat}           & 0  & 3 & 0    & 0.36 & -1    & -0.64 \\
\cellcolor{Gray}\textit{prevent}         & \cellcolor{Gray}0  & \cellcolor{Gray}1 & \cellcolor{Gray}0    & \cellcolor{Gray}0.12 & \cellcolor{Gray}-1    & \cellcolor{Gray}-0.88 \\
\textit{diagnose}        & 1  & 7 & 0.09 & 0.84 & -0.91 & 0.84 \\
\cellcolor{Gray}\textit{cause}           & \cellcolor{Gray}10 & \cellcolor{Gray}0 & \cellcolor{Gray}0.96 & \cellcolor{Gray}0    & \cellcolor{Gray}0.96  & \cellcolor{Gray}-1 \\
\textit{location}        & 1  & 0 & 0.09 & 0    & -0.91 & -1 \\
\cellcolor{Gray}\textit{symptom}         & \cellcolor{Gray}2  & \cellcolor{Gray}0 & \cellcolor{Gray}0.19 & \cellcolor{Gray}0    & \cellcolor{Gray}-0.81 & \cellcolor{Gray}-1 \\
\textit{manifestation}   & 0  & 0 & 0    & 0    & -1    & -1 \\
\cellcolor{Gray}\textit{contraindicate}  & \cellcolor{Gray}0  & \cellcolor{Gray}0 & \cellcolor{Gray}0    & \cellcolor{Gray}0    & \cellcolor{Gray}-1    & \cellcolor{Gray}-1 \\
\textit{associated with} & 1  & 3 & 0.09 & 0.36 & -0.91 & -0.64 \\
\cellcolor{Gray}\textit{side effect}     & \cellcolor{Gray}0  & \cellcolor{Gray}0 & \cellcolor{Gray}0    & \cellcolor{Gray}0    & \cellcolor{Gray}-1    & \cellcolor{Gray}-1 \\
\textit{is a}            & 0  & 0 & 0    & 0    & -1    & -1 \\
\cellcolor{Gray}\textit{part of}         & \cellcolor{Gray}0  & \cellcolor{Gray}0 & \cellcolor{Gray}0    & \cellcolor{Gray}0    & \cellcolor{Gray}-1    & \cellcolor{Gray}-1 \\
\textit{other}           & 0  & 1 & 0    & 0.12 & -1    & -0.88 \\
\cellcolor{Gray}\textit{none}            & \cellcolor{Gray}0  & \cellcolor{Gray}0 & \cellcolor{Gray}0    & \cellcolor{Gray}0    & \cellcolor{Gray}-1    & \cellcolor{Gray}-1 \\
%\hline
\end{tabular}
}
\end{table}
...
\hline
& \textbf{Dataset} & \textbf{P} & \textbf{P'} & \textbf{R} & \textbf{R'} & \textbf{F1} & \textbf{F1'}   \\ \hline \hline 
& \textit{crowd} & 0.565 & 0.632 & 0.743 & 0.754 & \textbf{0.642} & \textbf{0.687} \\ % & 0.784  &  \\ %\cline{2-6}
\textit{cause} & \textit{expert} & \textbf{0.672} & \textbf{0.711} & 0.604 & 0.616 & 0.638 & 0.658 \\ % & 0.818  &  \\ %\cline{2-6}
relation & \textit{baseline} & 0.436 & 0.474 & \textbf{0.844} & \textbf{0.842} & 0.575 & 0.606 \\ % & 0.674  &  \\ %\cline{2-6}
& \textit{single} & 0.495 & 0.545 & 0.473 & 0.478 & 0.483 & 0.658 \\ \hline % & 0.737  &  \\ \hline 
& \textit{crowd} & 0.823 & 0.843 & 0.891 & 0.902 & 0.854 & 0.869 \\ % & 0.854  & 0.868 \\ %\cline{2-6}
\textit{treat} & \textit{expert} & \textbf{0.834} & \textbf{0.863} & 0.833 & 0.84 & 0.832 & 0.85 \\ % & 0.839  & 0.855 \\ %\cline{2-6}
relation & \textit{baseline} & 0.767 & 0.811 & \textbf{0.968} & \textbf{0.968} & \textbf{0.856} & \textbf{0.882} \\ % & 0.844  & 0.874 \\ %\cline{2-6}
& \textit{single} & 0.774 & 0.819 & 0.856 & 0.866 & 0.811 & 0.84 \\ \hline % & 0.81 & 0.839 \\
\end{tabular}
}
\end{table}
...
\hline
& \textbf{Dataset} & \textbf{P} & \textbf{P'} & \textbf{R} & \textbf{R'} & \textbf{F1} & \textbf{F1'}   \\ \hline \hline
\textit{cause} & \textit{crowd} & \textbf{0.538} & \textbf{0.61} & 0.79  & 0.802 & \textbf{0.64}  & \textbf{0.692} \\ % & 0.768 & 0.803  \\ %\cline{2-10}
relation & \textit{baseline} & 0.475 & 0.53 & \textbf{0.889} & \textbf{0.887} & 0.619 & 0.663 \\ \hline %  & 0.715 & 0.751 \\ \hline 
\textit{treat} & \textit{crowd} & \textbf{0.876} & \textbf{0.913} & \textbf{0.887} & \textbf{0.898} & \textbf{0.88} & \textbf{0.904} \\ %  & 0.884 & 0.907  \\ %\cline{2-10}
relation & \textit{baseline} & 0.808 & 0.858 & 0.678 & 0.673 & 0.736 & 0.754 \\ \hline %  & 0.767 & 0.783  \\
\end{tabular}
}
\end{table}
...
%\hline
\multirow{2}{*}{\textbf{Sentence}} & \multirow{2}{*}{\textbf{Relation}} & \textbf{Crowd Label} & \textbf{Expert Label} \\ \hline  \hline
...
If the oxygen is too low, the incidence of \textbf{decompression sickness} increases; if the \textbf{oxygen} is too high, oxygen poisoning becomes a problem. & $cause$ & 0.743 & -1 \\ \hline
...
This is the first case of \textbf{Aicardi Syndrome} associated with lipoma and \textbf{metastatic angiosarcoma}. & $cause$ & -0.909 & 1 \\ \hline
Will giving \textbf{Acetaminophen} prevent the \textbf{pain} of the immunization? & $treat$ & 0.995 & -1 \\ \hline
FDA approves \textbf{Thalidomide} for \textbf{Hansen's disease} side effect, imposes unprecedented restrictions on distribution. & $treat$ & 0.913 & -1 \\ % \hline
\end{tabular}
\egroup
}
\end{table}
...
In contrast, we did not find a definite increase in the LGL percentage within 6 months postpartum in patients with \textbf{Grave's disease} who relapsed into \textbf{Grave's thyrotoxicosis}. & $cause$ & 0.737 & -1 \\ \hline
...
The diagnosis of \textbf{gyrate atrophy} was confirmed biochemically and clinically; \textbf{hyperornithinemia} and a deficiency of ornithine ketoacid transamise were confirmed biochemically. & $cause$ & -0.863 & 1 \\ \hline
...
The 1 placebo controlled trial that found black cohosh to be effective for \textbf{hot flashes} did not find \textbf{estrogen} to be effective, which casts doubt on the study's validity. & $treat$ & 0.73 & -1 \\ \hline
...
Patients with severe forms of \textbf{Von Willebrands' Disease (VWD)} may have frequent haemarthroses, especially when \textbf{Factor VIII (FVIII)} levels are below 10 U/dL, so that some of them develop target joints like patients with severe haemophilia A. & $treat$ & -1 & 1 \\ %\hline
\end{tabular}
\egroup
}
\end{table}
...
\textbf{Atopic conditions} include allergic rhinitis, atopic eczema, \textbf{allergic conjunctivitis} and asthma. & $cause$ & 0.507 & -1 \\ \hline
...
A retrospective review of 64 patients with haematuria and subsequent histologically proven \textbf{carcinoma of the bladder} revealed that bladder tumours could be diagnosed pre operatively in 34 of 46 (76\%) of patients with gross \textbf{haematuria} and 12 of of 18 (67\%) of those with microhaematuria. & $cause$ & -0.658 & 1 \\ \hline
...
To determine whether late asthmatic reactions and the associated increase in airway responsiveness induced by toluene diisocyate (TDI) are linked to \textbf{airway inflammation} we investigated whether they are inhibited by \textbf{Prednisone}. & $treat$ & 0.52 & -1 \\ \hline
