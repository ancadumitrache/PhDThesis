...
This chapter is in publication as ``Empirical Methodology for Crowdsourcing Ground Truth'' in the Semantic Web Journal~(\citet{dumitracheempirical}) and was co-authored by Oana Inel, Benjamin Timmermans, Carlos Ortiz, Robert-Jan Sips, Lora Aroyo and Chris Welty.
...
\item \textbf{comparative analysis of crowdsourcing aggregation methods:} we compare the performance of \textit{ambiguity-aware metrics} and \textit{consensus - enforcing metrics} over a diverse set of crowdsourcing tasks (Sections \ref{sec:results}, \ref{sec:discussion});

\item \textbf{stability of crowd results:} we show in several crowdsourcing tasks that \textit{an increased number of crowd workers leads to growth and stabilization in the quality of annotations}, going against the usual practice of employing a small number of annotators (Sections \ref{sec:results}, \ref{sec:discussion});

\item \textbf{measuring quality in open-ended tasks:} we present an extension to the CrowdTruth methodology that allows the ambiguity-aware metrics to deal \textit{both with open-ended and closed tasks} (Sections \ref{sec:methodology}, \ref{sec:experimental_setup}), as opposed to the initial version of the CrowdTruth metrics which only processed closed tasks;

\item \textbf{semantics of ambiguity:} applying the CrowdTruth methodology we collect richer data that allows to reason about ambiguity of content (in all modality formats, e.g. images, videos and sounds), which is intrinsically relevant to the Semantic Web community.
...
\begin{table}[!htp]
	...
	\begin{tabular}{rccl}
    %\hline
    \textbf{Task} & \textbf{Type} & \textbf{Media Unit} & \textbf{Annotations} \\
    \hline \hline
...
\begin{table}[!htp]
    ...
    %\hline
    \textbf{Task} & \textbf{Source} & \textbf{Has} & \textbf{Media} & \textbf{Workers/} & \textbf{Cost/} \\
    &  & \textbf{Expert} & \textbf{Units} & \textbf{Unit} & \textbf{Judgment} \\
    \hline \hline
    Medical Relation Extraction & PubMed & yes & 975 & 15 & \$0.05  \\
    Twitter Event Identification & Twitter (2014) & no & \np{3019} & 7 & \$0.02 \\
    News Event Extraction & TimeBank & yes & \np{200} & 15 & \$0.02 \\
    Sound Interpretation & Freesound.org & yes  & \np{284} & 10 & \$0.01 \\ %\hline
    \end{tabular}
    }
...
%\hline
\textbf{Task} & \textbf{Dataset} & \textbf{Precision} & \textbf{Recall} & \textbf{F1 score} & \textbf{Accuracy} &  \textbf{Threshold} \\ \hline \hline

\multirow{4}{*}{\parbox{1.8cm}{\textbf{Medical \\ Relation \\ Extraction}}} & CrowdTruth & 0.86 & 0.962 & 0.908 & 0.932 & \multirow{4}{*}{0.6} \\
 & expert & 0.899 & 0.89 & 0.895 & 0.927 &  \\
 & majority vote & 0.924 & 0.781 & 0.847 & 0.902 & \\
 & single & 0.222 & 0.776 & 0.346 & 0.748 & \\ \hline

\multirow{3}{*}{\parbox{1.8cm}{\textbf{Twitter \\ Event \\ Identification}}} & CrowdTruth & 0.965 & 0.945 & 0.955 & 0.995 & \multirow{3}{*}{0.4} \\
 & majority vote & 0.984 & 0.885 & 0.932 & 0.984 & \\
 & single & 0.959 & 0.819 & 0.884 & 0.972 & \\ \hline

\multirow{4}{*}{\parbox{1.8cm}{\textbf{News \\ Event \\ Extraction}}} & CrowdTruth & 0.984 & 0.929 & 0.956 & 0.931 & \multirow{4}{*}{0.05} \\
 & expert & 0.983 & 0.944 & 0.963 & 0.942 & \\
 & majority vote & 0.985 & 0.375 & 0.544 & 0.492 & \\
 & single & 0.99 & 0.384 & 0.554 & 0.501 & \\ \hline

\multirow{4}{*}{\parbox{1.8cm}{\textbf{Sound \\ Inter- \\ pretation}}} & CrowdTruth & 1 & 0.729 & 0.843 & 0.815 & \multirow{4}{*}{0.1} \\
...
\begin{tabular}{rccc}
%\hline
\textbf{Task} & \textbf{Maj. Vote} & \textbf{Expert} & \textbf{Single} \\ \hline \hline
...
%\hline
\multirow{2}{*}{\textbf{Media Unit}} & \multirow{2}{*}{\textbf{Annotation}} & \textbf{Expert} & \textbf{Crowd} & \textbf{Trusted} \\ 
 & & \textbf{Judgment} & \textbf{Score} & \textbf{Judgment} \\ \hline \hline
...
For empiric $treat$ment of epididymitis, especially when gonococcal or \textit{chlamydial infection} is likely Ofloxacin or \textit{levofloxacin} should be used  only  if epididymitis is not $cause$d by gonorrhea. & \multirow{6}{*}{$treat$} & \multirow{6}{*}{no} & \multirow{6}{*}{0.966} & \multirow{6}{*}{yes} \\ \hline 
...
The 1 placebo controlled trial that found black cohosh to be effective for \textit{hot flashes} did not find \textit{estrogen} to be effective, which casts doubt on the study's validity. & \multirow{5}{*}{$treat$} & \multirow{5}{*}{no} & \multirow{5}{*}{0.73} & \multirow{5}{*}{yes} \\ \hline 
...
Urokise versus \textit{tissue plasminogen activator} in \textit{pulmonary embolism}. & \multirow{2}{*}{$treat$} & \multirow{2}{*}{yes} & \multirow{2}{*}{0.365} & \multirow{2}{*}{no} \\ \hline 
...
These cases highlight the importance of considering \textit{PTLD} in the differential diagnosis of \textit{lymphadenopathy}. & \multirow{4}{*}{$cause$} & \multirow{4}{*}{yes} & \multirow{4}{*}{0.09} & \multirow{4}{*}{no} \\ %\hline 
\end{tabular}
}
\end{table}
...
Two Middle East terrorists with records of successful \textit{attacks} against Western targets Abu Nidal and Abu Abbas have ties to Baghdad. & \multirow{4}{*}{$attacks$} & \multirow{4}{*}{no} & \multirow{4}{*}{0.73} & \multirow{4}{*}{yes} \\ \hline 
...
The company also said it continues to explore all options concerning the possible \textit{sale} of National Aluminum's 54.5\% stake in an aluminum smelter in Hawesville Ky. & \multirow{5}{*}{$sale$} & \multirow{5}{*}{no} & \multirow{5}{*}{0.24} & \multirow{5}{*}{yes} \\ \hline 

Yield on the issue was 7.88\% & $no$ $event$ & yes & 0.14 & no \\ \hline 

Har-Shefi said she heard Amir talk about killing Rabin but did not tell the police because she did not believe he was \textit{serious}. & \multirow{4}{*}{$serious$} & \multirow{4}{*}{yes} & \multirow{4}{*}{0} & \multirow{4}{*}{no} \\ \hline 
...
%\hline
\multirow{2}{*}{\textbf{Media Unit URL}} & \textbf{Media Unit} & \multirow{2}{*}{\textbf{Annotation}} & \textbf{Expert} & \textbf{Crowd} & \textbf{Trusted} \\ 
 & \textbf{Description} &  & \textbf{Judgment} & \textbf{Score} & \textbf{Judgment} \\ \hline \hline

\multirow{4}{4cm}{\url{https://freesound.org/data/previews/21/21266_88803-hq.mp3}} & \multirow{4}{*}{jazz} & cymbals & no & 0.272 & yes \\
& & bangle & no & 0.136 & yes \\ 
& & rhythmic & no & 0.136 & yes \\ 
& &  &  & &  \\ \hline

\multirow{4}{4cm}{\url{https://freesound.org/data/previews/26/26086_11477-hq.mp3}} & \multirow{4}{*}{chicken} & birds & no & 0.538 & yes \\
& & geese & no & 0.359 & yes \\ 
& & horns & no & 0.359 & yes \\ 
& &  &  & &  \\ \hline

\multirow{4}{4cm}{\url{https://freesound.org/data/previews/35/35823_317782-hq.mp3}} & \multirow{4}{*}{weird drums} & music & no & 0.875 & yes \\
& & band & no & 0.145 & yes \\ 
& & disco & no & 0.145 & yes \\ 
& &  &  & &  \\ \hline

\multirow{4}{4cm}{\url{https://freesound.org/data/previews/39/39329_404624-hq.mp3}} & \multirow{4}{*}{trip hop} & beat & no & 0.371 & yes \\
& & percussion & no & 0.371 & yes \\ 
& & chimes & no & 0.371 & yes \\
& &  &  & &  \\ \hline

\multirow{4}{4cm}{\url{https://freesound.org/data/previews/41/41462_78779-hq.mp3}} & \multirow{4}{*}{beer glasses} & clicks & no & 0.242 & yes \\
& & clink & no & 0.242 & yes \\ 
& & ding & no & 0.242 & yes \\
& &  &  & &  \\ %\hline
\end{tabular}
